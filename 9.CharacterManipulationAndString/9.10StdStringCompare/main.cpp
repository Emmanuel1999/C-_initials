...
int main(){
    ...
    //Compare(1)
    //int compare (const basic_string& str) const noexpect; (1)
    //Compares this str to str
    /*
    ...
    std::cout << "str1.compare(str2) : " << str1.compare(str2) << std::endl;//-1
    std::cout << "str2.compare(str1) : " << str2.compare(str1) << std::endl;//1
    */
    ...
    //NB: When comparing if going up from a - z it's negative; but from z - a it's positive.
    */
    
    //Compare(3)
    //int compare (const char * s )const;(4)
    //Compares this string to the null-terminated character sequence beginning at the character pointed
    // to by s
