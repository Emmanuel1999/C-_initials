...
int main(){
    ...
    char dest[50] = "Hello ";
    char str[50] = "World!";
    std::strcat (dest, str); // Some compilers (MSVC) think these functions are unsafe!
    ...
    */
    ...
    //We can uses the returned pointer immediately for print out
    //This is a pattern you'll see a lot in C++ code out there

    std::cout << std::strncat(dest2, source2, 6) << std::endl;
    //Or std::strncat can be done separately and print out dest2
    ...
    */
    ...
    char *dest3 = new char [std::strlen(source3) +1]; //+1 for the null character containing
                                                    //garbage data uninitialized.
