...
int main(){
    ...
    */
    ...
    //Using std::size to querry the size of array dimensions
    /*
    for (size_t i = 0; i < std::size(packages); i++)
    {
        for (size_t j = 0; j < std::size(packages); ++j)
        {
            std::cout << packages [i] [j] << "  " ;
        }
        std::cout << std::endl;
    ...
    */
    ...
    //3D arrays are defined in the same way. We use 3 sets of indexes
    ...
    */
    ...
    int packages2 [] [5] {
        ...
        {7,8,9},
        {10,11,12},
        {13,14,15} //Can add as many triplets in packages2 as we want
    };

    std::cout << std::endl;
    std::cout << "Omiting leftmost dimension for 2D array : " << std::endl;
    ...
    */
    ...
    //For 3D and ANY multidimensional array, you MUST specify the 
    //number of elements in []'s, only the leftmost is not mandatory. 
    //Below is an example for 3D reproduced. Omitting the 5 or the 3
    //or both would cause compiler error.
    ...
    for (size_t i {}; i < std::size(block_house); ++i)
    {
        for (size_t j {}; j < std::size(block_house[i]); ++j)
        {
            std::cout << "[";
            for (size_t k {}; k < std::size(block_house[i][j]); ++k)
            {
                std::cout << block_house [i] [j] [k] << "    ";
            }
