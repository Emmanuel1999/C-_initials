...
int main(){
    ...
    std::optional<std::string> name {"Opatola Emmanuel"};
    std::optional<double> weight {88.3};
    std::optional<char> character {'f'};

    //Declare and empty initialize 
    std::optional<std::string> dog_name {}; //Initializes to std::nullopt
    std::optional<int> age {std::nullopt}; //std::nullopt is the null or zero equilvalent 
                                            //for std::optional
    ...
    std::cout << std::endl;
    std::cout << "Writing data in ..." << std::endl;
    ...
    //std::cout << height.value() << std::endl; //Throws exception and crashes program

    //has_value()
    std::cout << std::endl;
    std::cout << std::boolalpha;
    std::cout << "letter.has_value() : " << letter.has_value() << std::endl;//false
    std::cout << "height.has_value() : " << height.has_value() << std::endl;//false
    std::cout << "dog_name.has_value() : " << dog_name.has_value() << std::endl;//true
