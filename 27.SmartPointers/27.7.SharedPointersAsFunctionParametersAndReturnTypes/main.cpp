...
void use_dog_v1 (std::shared_ptr<Dog> dog) {
    std::cout << "shared_ptr passes by value, dog_name: " << dog->get_name() << std::endl;
    std::cout << "use count in use_dog_v1: " << dog.use_count() << std::endl; //2
}

//Pasing by non const reference
void use_dog_v2(std::shared_ptr<Dog> &dog) {
    //since no copy is made, we wont see the reference count increment here
    dog->set_dog_name("Riola");
    //dog.reset(new Dog()); //Passed by non const ref
    std::cout << "shared_ptr passed by non const reference (dog name chaneged in function), dog_name: " << dog->get_name() << std::endl;
...
void use_dog_v3(const std::shared_ptr<Dog> & dog) {
    ...
    dog->set_dog_name("Slimmy"); //We can change the dog object even though
                                //shared_ptr is passed by ref. The const protects the shared_ptr
                                //object itself, not the pointed object.
    
    //dog.reset(new Dog()); //Passed by const ref... Compiler error
    std::cout << "shared_ptr passed by const ref (dog name changed in function), dog_name: " << dog->get_name() << std::endl;
...
int main(){
    ...
    std::shared_ptr<Dog> shared_ptr_dog_1 = std::make_shared<Dog> ("Fillo");
